...
* http://jqueryui.com
* Includes: jquery.ui.core.js, jquery.ui.widget.js, jquery.ui.mouse.js, jquery.ui.position.js, jquery.ui.draggable.js, jquery.ui.sortable.js, jquery.ui.autocomplete.js, jquery.ui.datepicker.js, jquery.ui.menu.js, jquery.ui.slider.js, jquery.ui.tabs.js, jquery.ui.tooltip.js
...
(function($) {
  ...
  $.fn.bPopup = function(options, callback) {
    ...
    var $popup = this,
      ...
      buffer = 200,
      popups = 0,
      id, inside, fixedVPos, fixedHPos, fixedPosStyle, vPos, hPos, height, width, debounce, hasNano = false;
    ...
    function init() {
      ...
      popups = ($w.data('bPopup') || 0) + 1, id = prefix + popups + '__', fixedVPos = o.position[1] !== 'auto', fixedHPos = o.position[0] !== 'auto', fixedPosStyle = o.positionStyle === 'fixed';
    ...
    function open() {
      ...
      $popup.each(function() {
        if (o.appending) {
          ...
          $this.appendTo(o.appendTo).data('bPopup', o);
    ...
    function alignPopup(){
      ...
      $popup.animate({
        left: hPos,
        top: vPos + (fixedPosStyle ? 0 : d.scrollTop())
      }, 100);
    }
    ...
    function close() {
      ...
      // Close trasition
      doTransition();
      $w.trigger('popupClose');
      if($popup.data('parent')){
        $popup.hide().appendTo($popup.data('parent'));
      }
    ...
    function bindEvents() {
      ...
      if (o.useNanoScroll == true && hasNano) {
        $popup.find('.nanowrap > .nano').nanoScroller();
      }
      alignPopup();
    };
...
/* --- moved from init: --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- */
      ...
      if (o.useNanoScroll == true && height > wH - o.amsl && !$popup.find('.nanowrap > .nano').length) {
        hasNano = true;
        $content.wrapInner('<div class="nanowrap"><div class="nano"><div class="nanocontent">');
        $popup.find('.nanowrap').height(wH - o.amsl - hH - 60)/*.width(width - 60)*/;
        $popup.find('.nanowrap > .nano').nanoScroller();
        height = $popup.outerHeight(true);
      } else if($popup.find('.nanowrap > .nano').length){
        $popup.find('.nanowrap').addClass('calc');
        var nH = $popup.find('.nanowrap').height();
        $popup.find('.nanowrap').removeClass('calc');
        if(nH < wH - o.amsl - hH - 60){
          $popup.find('.nanowrap > .nano > .pane').remove();
          $popup.find('.nanowrap > .nano > .nanocontent').removeClass('nanocontent').removeAttr('tabindex').addClass('content').unwrap().unwrap().unwrap();
        } else {
          $popup.find('.nanowrap').height(wH - o.amsl - hH - 60)/*.width(width - 60)*/;
          $popup.find('.nanowrap > .nano').nanoScroller();
        }
...
/* --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- */
    ...
    function calPosition() {
      ...
      vPos = fixedVPos ? o.position[1] : Math.max(0, ((wH - $popup.outerHeight(true)) / 2)), hPos = fixedHPos ? o.position[0] : (wW - $popup.outerWidth(true)) / 2, inside = insideWindow();
    };
