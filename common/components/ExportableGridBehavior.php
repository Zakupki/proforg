...
class ExportableGridBehavior extends CBehavior {
    ...
    public function exportCSVDisposition($data, $attributes = array(), $endApplication = true, $endLineCount = 0, $otherdata=null) {
        //if ($this->isExportRequest()) {
        $this->sendHeaders();
        $fileHandle = fopen('php://output', 'w');
        if ($data instanceof CActiveDataProvider) {
            ...
            $this->csvRowHeaders($fileHandle, $attributes, current($data));
            $this->csvRowModelsDisposition($fileHandle, $data, $attributes, $otherdata);
        } else if (is_array($data) && is_string(current($data))) {
            fputcsv($fileHandle, $data, $this->csvDelimiter, $this->csvEnclosure);
        } else if ($data instanceof CModel) {
            $this->csvModel($fileHandle, $data, $attributes);
    ...
    private function csvRowModels($fileHandle, $cModels, $attributes) {
        foreach ($cModels as $cModel) {
            $row = array();
            foreach ($attributes as $attr) {
                $row[] = str_replace('.',',',CHtml::value($cModel, $attr));
            }
            fputcsv($fileHandle, $row, $this->csvDelimiter, $this->csvEnclosure);
        }
    }
    private function csvRowModelsDisposition($fileHandle, $cModels, $attributes,$otherdata) {
        foreach ($cModels as $cModel) {
            $row = array();
            foreach ($attributes as $attr) {
                if($attr=='price_diff'){
                //$otherdata[CHtml::value($cModel, 'id')][CHtml::value($cModel, 'company_id')][CHtml::value($cModel, 'delivery_id')];
                if(isset($otherdata[CHtml::value($cModel, 'id')][CHtml::value($cModel, 'company_id')][CHtml::value($cModel, 'delivery_id')]))
                    if(CHtml::value($cModel, 'avg_price')>$otherdata[CHtml::value($cModel, 'id')][CHtml::value($cModel, 'company_id')][CHtml::value($cModel, 'delivery_id')])
                        $row[] = '+'.str_replace('.',',',CHtml::value($cModel, 'avg_price'))-str_replace('.',',',$otherdata[CHtml::value($cModel, 'id')][CHtml::value($cModel, 'company_id')][CHtml::value($cModel, 'delivery_id')]);
                    else
                        $row[] = str_replace('.',',',CHtml::value($cModel, 'avg_price'))-str_replace('.',',',$otherdata[CHtml::value($cModel, 'id')][CHtml::value($cModel, 'company_id')][CHtml::value($cModel, 'delivery_id')]);
                else
                    $row[] ='';
    ...
    private function sendHeaders() {
        if ($this->headersSent === false) {
            ...
            header("Expires: Tue, 01 Jan 2015 00:00:00 GMT");
